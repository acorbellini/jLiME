	...
	public void load(Metrics mgr) {
		...
		mgr.createTimedSensor(new TimedSet() {
			...
			public Set<String> updateSet(IMetrics mgr) throws Exception {
				...
				String iorawdata = CommandLineUtils.execCommand("iostat -p -d | tail -n+4 | tr -s ' '").trim();
				BufferedReader reader = new BufferedReader(new StringReader(iorawdata));
				...
				while ((line = reader.readLine()) != null) {
					...
					mgr.gauge(k + ".tps").update(Float.valueOf(iodata[1].replaceAll(",", ".")));
					mgr.gauge(k + ".read").update(Float.valueOf(iodata[2].replaceAll(",", ".")));
					mgr.gauge(k + ".write").update(Float.valueOf(iodata[3].replaceAll(",", ".")));
					mgr.gauge(k + ".read_total").update(Float.valueOf(iodata[4].replaceAll(",", ".")));
					mgr.gauge(k + ".write_total").update(Float.valueOf(iodata[5].replaceAll(",", ".")));
					current.add(k);
				}
				return current;
