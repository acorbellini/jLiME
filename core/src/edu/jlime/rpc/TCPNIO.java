...
public class TCPNIO extends MessageProcessor implements AddressListProvider {
	...
	public void onStart() throws Exception {
		for (int i = 0; i < config.port_range; i++) {
			try {
				...
				this.channel.bind(new InetSocketAddress(
						InetAddress.getByName(iface), config.port + i));
				break;
			} catch (Exception e) {
				// e.printStackTrace();
			}
	...
	private void read(SocketChannel channel) throws Exception {
		...
		Message msg = Message.deEncapsulate(buff.getRawByteArray(), from,
				local);
		try {
			notifyRcvd(msg);
		} catch (Exception e) {
