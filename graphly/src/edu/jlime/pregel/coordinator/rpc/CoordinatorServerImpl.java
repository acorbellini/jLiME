...
public class CoordinatorServerImpl extends RPCClient
		implements Coordinator, Transferible {

	transient RPC localRPC;
	transient volatile Coordinator local = null;

	public CoordinatorServerImpl(RPC disp, Peer dest, Peer client,
			String targetID) {
	...
	public void error(final Exception arg0, final int arg1, final UUID arg2)
			throws Exception {
		if (localRPC != null) {
			async.execute(new Runnable() {
				public void run() {
					try {
						getLocal().error(arg0, arg1, arg2);
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
		...
		disp.callAsync(dest, client, targetID, "error",
