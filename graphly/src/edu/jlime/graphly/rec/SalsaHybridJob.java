...
public class SalsaHybridJob implements Job<AuthHubSubResult> {
	...
	public SalsaHybridJob(Graph graph, int authSize, int hubSize, String auth, String hub, int i,
			TLongHashSet subgraph, TLongArrayList value) {
				...
				public AuthHubSubResult call() throws Exception {
					...
					while (cont < to) {
						...
						if (step % 2 == 0) {
							...
							if (inW.length > 0) {
								float a = g.getFloat(v, auth, 1f / authSize);
							...
							if (outW.length > 0) {
								float h = g.getFloat(v, hub, 1f / hubSize);
